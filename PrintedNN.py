...
class pLayer(torch.nn.Module):
    def __init__(self, n_in, n_out, args, ACT, INV):
        ...
        theta[-2, :] = self.ACT.eta[2].detach().item() / (1.-self.ACT.eta[2].detach().item()) * (torch.sum(theta[:-2, :], axis=0)+theta[-1, :])
    ...
    @property
    def power(self):
        return self.mac_power + self.act_power + self.neg_power / 1000.
...
class LossFN(torch.nn.Module):
    ...
    def forward(self, model, x, label):
        ...
        if self.args.loss == 'pnnloss':
            return self.standard(prediction, label) + 0.1 * model.power
        elif self.args.loss == 'celoss':
            return self.celoss(prediction, label) + 0.1 * model.power
...
class LFLoss(torch.nn.Module):
    ...
    def forward(self, model, x, label):
        ...
        return torch.stack(L).mean() + 0.01 * model.power
