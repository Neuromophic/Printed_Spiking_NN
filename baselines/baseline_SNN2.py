...
for ds_idx in range(len(datasets)):
    ...
    setup = f'{name}_{seed}.snn2'
    
    if os.path.exists(f'./baseline_result/SNN2/{setup}'):
        print(f'{setup} exists.')

    else:
        torch.manual_seed(seed)

        model = T.SNN2(N_channel, 3, N_class)
        ...
        if not os.path.exists('./baseline_result/SNN2/'):
            os.makedirs('./baseline_result/SNN2/')
        torch.save(package, f'./baseline_result/SNN2/{setup}')
