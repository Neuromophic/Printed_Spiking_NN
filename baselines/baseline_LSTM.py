...
for ds_idx in range(len(datasets)):
    ...
    setup = f'{name}_{seed}.lstm'
    
    if os.path.exists(f'./baseline_result/LSTM/{setup}'):
        print(f'{setup} exists.')

    else:
        torch.manual_seed(seed)

        model = T.lstm(N_channel, N_class)
        pprint(model)
        ...
        if not os.path.exists('./baseline_result/LSTM/'):
            os.makedirs('./baseline_result/LSTM/')
        torch.save(package, f'./baseline_result/LSTM/{setup}')
